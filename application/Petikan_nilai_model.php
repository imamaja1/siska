...
class Petikan_nilai_model extends CI_Model {
    ...
    public function petikan_nilai($nim, $kode_nama_kurikulum) {
        ...
        $t_sksn = 0;
        for ($j = 1; $j <= 8; $j++) {
//            $data_kurikulum = $this->db->query("SELECT * FROM kurikulum, matakuliah WHERE kode_nama_kurikulum='{$kode_nama_kurikulum}' and kurikulum.kode_matakuliah=matakuliah.kode_matakuliah and semester='{$j}'")->result();
            $data_kurikulum = $this->db->select('*')
                            ->from('kurikulum as kur')
//                ->join('matakuliah as mak','kur.kode_matakuliah=mak.kode_matakuliah')
...
//                    if (stup_grade($kode_kurikulum_angkatan, $cek->semester))
//                    {
//                        $data_penilaian = stup_grade($kode_kurikulum_angkatan, $cek->semester);
//                    }else{
...
//                    foreach ($lebih as $item) {
                  	if($cek->nama_matakuliah == 'Skripsi'){
                      		$lebih = $this->db->select('*')
                                    ...
                                    ->limit(1)
                                    ->get();
                      	$data[$n]['data_nilai'][$i]['semester'] = $lebih->row_object()->semester;
                    	$data[$n]['data_nilai'][$i]['nilai_uts'] = $lebih->row_object()->nilai_uts;
                    	$data[$n]['data_nilai'][$i]['nilai_uas'] = $lebih->row_object()->nilai_uas;
                    	$data[$n]['data_nilai'][$i]['nilai_akhir'] = $lebih->row_object()->nilai_akhir;
                    }else{
                        $lebih = $this->db->select('*')
                                ->from('krs')
                                ->join('krs_detail as kd', 'krs.kode_krs=kd.kode_krs')
                                ...
                                ->where('nim', $nim)
                                ->where('mak.kode_matakuliah', $row->kode_matakuliah)
                                ->order_by('krs.date_created', 'DESC')
                                ->get();
                        ...
                        $semester = $this->db->select('*')
                                ...
                                ->row();
                        $tmp_nilai = 0;
                        foreach ($lebih->result_object() as $key => $value) {
                            if($value->semester == $semester->semester){
                                if($value->nilai_akhir == 0 || $value->nilai_akhir == null){
                                    if ($tmp_nilai < $value->nilai_akhir) {
                                        $data[$n]['data_nilai'][$i]['semester'] = $value->semester;
                                        $data[$n]['data_nilai'][$i]['nilai_uts'] = $value->nilai_uts;
                                        $data[$n]['data_nilai'][$i]['nilai_uas'] = $value->nilai_uas;
                                        $data[$n]['data_nilai'][$i]['nilai_akhir'] = $value->nilai_akhir;
                                        $tmp_nilai = $value->nilai_akhir;
                                    }
                                    break;
                                }else{
                                    if ($tmp_nilai < $value->nilai_akhir) {
                    ...
                    $data_penilaian = data_penilaian($nim, $lebih->row_object()->semester);
//                        if (stup_grade($kode_kurikulum_angkatan, $lebih->semester))
//                        {
//                            $data_penilaian = stup_grade($kode_kurikulum_angkatan, $lebih->semester);
//                        }else{
//                            $data_penilaian = sistem_penilaian($nim);
//                        }
                    $data[$n]['data_nilai'][$i]['sks'] = $sks;
                    $nilai_akhir = $data[$n]['data_nilai'][$i]['nilai_akhir'] * 1;
...
//
    ...
    public function get_kode_krs_detail($kode_krs) {
        ...
        $query = $this->db->query("SELECT *, krs.semester FROM krs, krs_detail, khs_detail, matakuliah WHERE krs_detail.kode_krs=krs.kode_krs and krs_detail.kode_krs_detail=khs_detail.kode_krs_detail and krs_detail.id_matakuliah=matakuliah.id_matakuliah and krs_detail.kode_krs=" . $kode_krs . "")->result();
        return $query;
    }
