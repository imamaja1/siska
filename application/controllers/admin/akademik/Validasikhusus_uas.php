...
class validasikhusus_uas extends CI_Controller {
    ...
    public function index() {
        ...
        $ta = $this->m_tahun_akademik->get_nilai_not_26();
...
//        $kode_tahun_akademik = tahun_akademik()->kode_tahun_akademik;
        $kelas = $this->validasiservice->get_kelas_uas_by_prodi($kode_prodi);
        foreach ($kelas as $key => $value) {
            $kelas[$key]->jum_mhs = $this->validasiservice->count_mahasiswa_in_kelas($value->kelas_id);
        }
        $data['content'] = 'admin/akademik/nilai/V_validasi_khusus_uas';
        $data['judul'] = 'Akademik';
...
//        $id_dekan = $this->session->userdata('kode_dosen');
        ...
        $kode_prodi = array_column($prodi, 'kode_program_studi');
//        $kode_tahun_akademik = tahun_akademik()->kode_tahun_akademik;
        $kelas = $this->validasiservice->get_kelas_uas_by_prodi_and_ta($kode_prodi, $kode_tahun_akademik);
