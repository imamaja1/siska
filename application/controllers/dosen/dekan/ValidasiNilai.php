...
class ValidasiNilai extends CI_Controller {
    ...
    public function index() {
        ...
        $kelas = $this->dekanservice->get_kelas_validasi($kode_prodi, $kode_tahun_akademik);
    ...
    public function lihat_data_mahasiswa_uts($kelas_id) {
        ...
        $this->load->view('dosen/dekan/V_lihat_nilai_uts', $data);
    }
    ...
    public function lihat_data_mahasiswa_uas($kelas_id) {
        ...
        $this->load->view('dosen/dekan/V_lihat_nilai_uas', $data);
    }

    public function revisi_uas($kelas_id = null) {
    ...
    public function revisi_uts($kelas_id = null) {
        $note = htmlspecialchars($this->input->post('catatan_prodi'));
        if ($kelas_id === null) {
            $kelas_id = htmlspecialchars($this->input->post('kelas_id'));
        }
   ...
   public function validasi_nilai_uas() {
    ...
    public function validasi_nilai_revisi($ta = null) {
        ...
        $kelas = $this->dekanservice->get_kelas_revisi_dekan($kode_prodi, $kode_tahun_akademik);

        foreach ($kelas as $key => $value) {
            $num = $this->dekanservice->get_dummy_update_kelas_status($value->kelas_id, '1');

            $tmp = $this->dekanservice->get_dummy_update_kelas_row($value->kelas_id, $num, 0);

            $nama_dosen = $this->dekanservice->get_nama_dosen_kelas($value->kelas_id);
    ...
    public function choose_nilai_revisi() {
        ...
        foreach ($kelas as $key => $value) {
            $num = $this->dekanservice->get_dummy_update_kelas_status($value->kelas_id, '1');
            $num2 = $this->dekanservice->get_dummy_update_kelas_status($value->kelas_id, '2');

            $tmp = $this->dekanservice->get_dummy_update_kelas_row($value->kelas_id, $num, $num2);
    ...
    public function revisi_nilai_selesai(){
        $kelas = $this->input->POST('kelas');
        $level = $this->input->POST('level');
        $this->dekanservice->update('dummy_update_kelas', array('status_dekan' => 'T','status' => '3'), array('id_kelas' => $kelas, 'level' => $level));
        $nilai = $this->dekanservice->get_dummy_update_nilai($kelas, $level);
        foreach ($nilai as $row) {
            $data_nilai = array(
    ...
    public function revisi_nilai_all_mahasiswa(){
        ...
        foreach ($saber as $key => $value) {
            $saber[$key]->nilai_mhs = $this->dekanservice->get_nilai_revisi_all_dekan($kelas, $value->level);
        }
