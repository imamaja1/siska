...
class validasinilai extends CI_Controller {
    ...
    public function kelas_uas() {
        ...
        $kode_prodi = array_column($prodi, 'kode_program_studi');
        $kode_tahun_akademik = tahun_akademik()->kode_tahun_akademik;
        $kelas = $this->dekanservice->get_kelas_uas_dekan_simple($kode_prodi, $kode_tahun_akademik);
        $data['kelas'] = $kelas;
   ...
   public function validasi_nilai_uas() {
        $prodi = $this->dekanservice->get_dekan_prodi($this->session->userdata('kode_dosen'));
