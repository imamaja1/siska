...
class Validasinilai extends CI_Controller {
    ...
    public function validasi_nilai_revisi() {
        ...
        $kelas = $this->kaprodiservice->get_kelas_revisi($kode_prodi, $kode_tahun_akademik);
    
        foreach ($kelas as $key => $value) {
            $num = $this->kaprodiservice->get_dummy_update_kelas_status($value->kelas_id, '1');

            $tmp = $this->kaprodiservice->get_dummy_update_kelas_row($value->kelas_id, $num, 0);

            $nama_dosen = $this->kaprodiservice->get_nama_dosen_by_kelas($value->kelas_id);

            $kelas[$key]->data_kelas = $tmp;
            $kelas[$key]->dosen_pengampu = $nama_dosen;
        }
        $data['content'] = 'dosen/kaprodi/validasi-nilai/V_index_revisi';
        $data['judul'] = 'Validas Nilai Revisi';
        $data['kelas'] = $kelas;
    ...
    public function choose_nilai_revisi() {
        ...
        $prodi = $this->kaprodiservice->get_kaprodi_prodi_array($this->session->userdata('kode_dosen'));
        $kode_prodi = array_column($prodi, 'kode_program_studi');
        $kelas = $this->kaprodiservice->get_kelas_revisi_order($kode_prodi, $kode_tahun_akademik);
    
        foreach ($kelas as $key => $value) {
            $num = $this->kaprodiservice->get_dummy_update_kelas_status($value->kelas_id, '1');
            $num2 = $this->kaprodiservice->get_dummy_update_kelas_status($value->kelas_id, '2');

            $tmp = $this->kaprodiservice->get_dummy_update_kelas_row($value->kelas_id, $num, $num2);
    ...
    public function revisi_nilai_validasi(){
        ...
        $data['data'] = $this->kaprodiservice->get_revisi_nilai_validasi($kelas, $level, $kode_ta);
        $data['nilai'] = true;

        $data['kelas'] = $this->kaprodiservice->get_revisi_nilai_kelas($kelas, $level);
        $this->load->view('dosen/kaprodi/validasi-nilai/V_revisi_nilai_mahasiswa', $data);
    }
    public function revisi_nilai_validasi_mhs(){
        $kelas = $this->input->POST('kelas');
        $level = $this->input->POST('level');
   
        $data['data'] = $this->kaprodiservice->get_mahasiswa_kelas_only($kelas);
        $data['nilai'] = false;
    ...
    public function revisi_nilai_selesai(){
        ...
        $this->kaprodiservice->update('dummy_update_kelas', array('status_prodi' => 'T','status' => '2'), array('id_kelas' => $kelas, 'level' => $level));
    ...
    public function revisi_nilai_revisi(){
        ...
        $this->kaprodiservice->update('dummy_update_kelas', array('status_prodi' => 'R','status_dosen' => 'R','status' => '4'), array('id_kelas' => $kelas, 'level' => $level));
