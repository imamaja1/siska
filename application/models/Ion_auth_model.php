...
class Ion_auth_model extends CI_Model {
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select($this->identity_column . ', email, id_user, password, active, last_login')
                ->where($this->identity_column, $identity)
                ->limit(1)
                ->order_by('id_user', 'desc')
                ->get($this->tables['users']);
            ...
            if ($last_login + $recheck < time()) {
                ...
                if ($query->num_rows() === 1) {
                    ...
                    if (substr(CI_VERSION, 0, 1) == '2') {
                        $this->session->unset_userdata(array($identity => '', 'id_user' => '', 'id_user' => ''));
                    } else {
                        $this->session->unset_userdata(array($identity, 'id_user', 'id_user'));
                    }
                    return false;
    ...
    public function users($groups = NULL) {
        ...
        if (isset($this->_ion_select) && !empty($this->_ion_select)) {
            ...
            $this->db->select(array(
                $this->tables['users'] . '.*',
                $this->tables['users'] . '.id_user as id_user',
                $this->tables['users'] . '.id_user as id_user'
            ));
        }
        ...
        if (isset($groups)) {
            ...
            if (isset($groups) && !empty($groups)) {
                $this->db->distinct();
                $this->db->join(
                        $this->tables['users_groups'], $this->tables['users_groups'] . '.' . $this->join['users'] . '=' . $this->tables['users'] . '.id_user', 'inner'
                );
            }
            ...
            if (!empty($group_names)) {
                $this->db->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . ' = ' . $this->tables['groups'] . '.id_user', 'inner');
    ...
    public function get_users_groups($id = FALSE) {
        ...
        return $this->db->select($this->tables['users_groups'] . '.id_group as id_group, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description')
    ...
    public function add_to_group($group_ids, $user_id = false) {
        ...
        // if no id was passed use the current users id
        $user_id || $user_id = $this->session->userdata('id_user');
    ...
    public function update($id, array $data) {
        ...
        $this->trigger_events('extra_where');
        $this->db->update($this->tables['users'], $data, array('id_user' => $user->id));
    ...
    public function delete_user($id) {
        ...
        $this->db->delete($this->tables['users'], array('id_user' => $id));
    ...
    public function set_session($user) {
        ...
        $session_data = array(
            ...
            'email' => $user->email,
            'id_user' => $user->id, //everyone likes to overwrite id so we'll use user_id
    ...
    public function remember_user($id) {
        ...
        $this->db->update($this->tables['users'], array('remember_code' => $salt), array('id_user' => $id));
