...
class Kelas_kpat_model extends CI_Model
{
    private $table = 'kelas_kpat';
    ...
    function get_nama_kelas_by_kelas_id($kelas_id)
    {
        return $this->db->select('*')
            ->from('kelas_kpat')
            ->join('nama_kelas as nk','kelas_kpat.nama_kelas_id=nk.nama_kelas_id')
    ...
    {
        $this->db->insert('kelas_kpat', $data);
    ...
    {
        $query = $this->db->select('* ,count(kode_krs_detail) as jml,kelas_kpat.kelas_id')
            ->from('kelas_kpat')
            ->join('nama_kelas as nk', 'kelas_kpat.nama_kelas_id=nk.nama_kelas_id')
            ->join('kelas_mahasiswa_kpat as km', 'kelas_kpat.kelas_id=km.kelas_id','left')
            ->where('kode_tahun_akademik', $kode_tahun_akademik)
            ->where('kode_program_studi', $kode_program_studi)
            ->where('id_matakuliah', $id_matakuliah)
            ->group_by('kelas_kpat.nama_kelas_id')
            ->order_by('kelas_kpat.nama_kelas_id ASC')
            ->get()->result();
        return $query;
    }
    ...
    function get_matakuliah_combobox($kode_tahun_akademik, $kode_program_studi, $semester)
    {
        $query = $this->db->select('*')
            ->from('kelas_kpat')
            ->join('nama_kelas as nk', 'kelas_kpat.nama_kelas_id=nk.nama_kelas_id')
            ->join('matakuliah as mk', 'kelas_kpat.id_matakuliah=mk.id_matakuliah')
            ->where('kode_tahun_akademik', $kode_tahun_akademik)
            ->where('kelas_kpat.kode_program_studi', $kode_program_studi)
            ->where('substr(kelas_kpat.kode_matakuliah,6,1)', $semester)
            ->group_by('kelas_kpat.id_matakuliah')
            ->order_by('kelas_kpat.id_matakuliah ASC')
            ->get()->result();
        return $query;
    }
    ...
    function get_mahasiswa_kelas($kelas_id)
    {
        $query = $this->db->select('*')
            ->from('kelas_kpat')
            ->join('kelas_mahasiswa_kpat as km','km.kelas_id=kelas_kpat.kelas_id')
    ...
    {
        return $this->db->where('kelas_mahasiswa_id', $id)->update('kelas_mahasiswa_kpat', $data);
    }
...
//        $tahun_akademik = tahun_akademik()->kode_tahun_akademik;
    ...
    function hapus($id)
    {
        return $this->db->where('kelas_mahasiswa_id', $id)->delete('kelas_mahasiswa_kpat');
    }
