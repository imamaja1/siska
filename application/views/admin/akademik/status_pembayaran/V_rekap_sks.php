<div class="row">
    <div class="col-md-12">
        <div class="box box-solid ">
            <div class="box-body">
                <button onclick="ambil()" class="btn btn-warning btn-sm"><i class="fa fa-refresh"></i> Refresh</button>
                <button onclick="skripsi()" class="btn btn-danger btn-sm"><i class="fa fa-refresh"></i> SKS SKRIPSI</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function ambil() {
        $.ajax({
            url : "<?= site_url('admin/akademik/pembayaran_mahasiswa/filter_rekap_sks') ?>",
            beforeSend : function(){
                $("#landing").html(loading);
            },
            success : function (res) {
    ...
    function skripsi() {
        $.ajax({
            url : "<?= site_url('admin/akademik/pembayaran_mahasiswa/filter_rekap_sks_skripsi') ?>",
            beforeSend : function(){
                $("#landing").html(loading);
            },
            success : function (res) {
