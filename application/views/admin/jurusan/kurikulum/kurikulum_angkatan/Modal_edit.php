...
<form class="form-horizontal" method="POST" onsubmit="update(this, event)" action="<?= site_url('admin/jurusan/kurikulum/kurikulum_angkatan/update/'.$id) ?>">
    ...
    <div class="modal-body">
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <input type="text" required class="form-control" value="<?= e($data->angkatan) ?>" name="angkatan" placeholder="Ex: 2019, 2017">
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <select name="kode_nama_kurikulum" class="form-control select2" required style="width: 100%">
                    ...
                    <?php foreach ($nama_kurikulum as $row) : ?>
                        <option <?= $data->kode_nama_kurikulum == $row->kode_nama_kurikulum ? 'selected' : '' ?> value="<?= e($row->kode_nama_kurikulum) ?>"><?= e($row->nama_kurikulum) ?> - <?= e($row->singkatan_program_studi)?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <div class="radio">
                    <label><input type="radio" <?= $data->ekstensi == 'N' ? 'checked' : '' ?>  name="ekstensi" id="optionsRadios1" value="N" >Tidak</label>&nbsp;
                    <label><input type="radio" <?= $data->ekstensi == 'Y' ? 'checked' : '' ?> name="ekstensi" id="optionsRadios1" value="Y" >Ya</label>
                </div>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <div class="radio">
                    <label><input type="radio" <?= $data->paket == 'N' ? 'checked' : '' ?> name="paket" id="optionsRadios1" value="N" checked >Tidak</label>&nbsp;
                    <label><input type="radio" <?= $data->paket == 'Y' ? 'checked' : '' ?> name="paket" id="optionsRadios1" value="Y" >Ya</label>
                </div>
            </div>
        </div>
    </div>
