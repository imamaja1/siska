<div class="box box-solid flat">
    <div class="box-body">
        <button class="btn btn-primary btn-sm btn-flat" onclick="$('#modal-tambah').modal('toggle')"><i class="fa fa-plus-circle"></i> Tambah</button>
    </div>
</div>
<div id="landing"></div>
	...
	<input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <div class="radio">
                                <label><input type="radio" name="ekstensi" id="optionsRadios1" value="N" checked >Tidak</label>&nbsp;
                                <label><input type="radio" name="ekstensi" id="optionsRadios1" value="Y" >Ya</label>
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <div class="radio">
                                <label><input type="radio" name="paket" id="optionsRadios1" value="N" checked >Tidak</label>&nbsp;
                                <label><input type="radio" name="paket" id="optionsRadios1" value="Y" >Ya</label>
                            </div>
                        </div>
                    </div>
                </div>
...
<div class="modal fade" id="modal-edit" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content" id="content-edit"></div>
    </div>
</div>
<script>
            ...
            if (!$(this)[0].checkValidity()) { swal('Warning','Semua form required harus di isi','error');
                ...
                $.ajax({
                    ...
                    success : function (res) {
                        ...
                        if (obj.status) { all(); $('#modal-tambah').modal('hide'); swal('Success', obj.msg, 'success');
                        }else{ all(); $('#modal-tambah').modal('hide'); swal('Gagal', obj.msg, 'error'); }
                    }
                })
            }
        })
    ...
    function all() {
        ...
        $.ajax({ url : url, success : function (res) { $("#landing").html(res); } })
    }
    function edit(id) {
        var url = "<?= site_url('admin/jurusan/kurikulum/kurikulum_angkatan/edit') ?>/"+id;
        $.ajax({ url : url, success : function (res) { $("#content-edit").html(res); $("#modal-edit").modal('show'); } })
    }
        ...
        swal({
            ...
            $.ajax({
                url : url, success : function (res) {
                    var obj = JSON.parse(res);
                    if (obj.status) { all(); swal('Success!',obj.msg, 'success'); }else{ swal('Gagal!',obj.msg, 'error'); }
                }
            })
        });
    }
