<div class="row">
    ...
    <!--    <div class="col-md-5 col-xs-12" id="landing-nilai">-->
...
<script>
    ...
    function lihat(kelas_id) {
        ...
        var url = "<?= site_url('dosen/dekan/validasinilai/data_mahasiswa_uas') ?>/" + kelas_id;
        $.ajax({
            url: url,
    ...
    function validasi() {
        var url = "<?= site_url('dosen/dekan/validasinilai/validasi_uas') ?>/" + super_kelas_id;
        swal({
            title: '',
            html: "<strong>Validasi Nilai</strong> berarti anda sudah selesai mengoreksi nilai mahasiswa yang di inputkan dosen." +
                "Nilai yang sudah di <storng>Validasi</storng> tidak bisa dirubah kembali. Tekan <strong>YA</strong> untuk melanjutkan dan <strong>Tidak</strong> untuk membatalkan.",
            type: 'info',
    ...
    function revisi() {
        var url = "<?= site_url('dosen/dekan/validasinilai/revisi_uas') ?>/" + super_kelas_id;
        swal({
            title: '',
    ...
    function kelas() {
        var url = "<?= site_url('dosen/dekan/validasinilai/kelas_uas') ?>";
        $.ajax({
            url: url,
            beforeSend: function () {
    ...
    function lihat_mhs(kelas_id) {
        super_kelas_id = kelas_id;
        var url = "<?= site_url('dosen/dekan/validasinilai/lihat_data_mahasiswa_uas') ?>/" + kelas_id;
        $.ajax({
            url: url,
            success: function (res) {
                $("#landing-lihat").html(res);
