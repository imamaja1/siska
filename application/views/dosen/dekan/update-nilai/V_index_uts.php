<div class="row">
    <div class="col-md-12 col-xs-13" id="landing-kelas">
    </div>
</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><i class="fa fa-comments-o"></i> Komentar / Catatan</h4>
            </div>
...
<script>
    ...
    function lihat(kelas_id) {
        ...
        var url = "<?= site_url('dosen/dekan/update_penilaian/data_mahasiswa_uts') ?>/" + kelas_id;
        $.ajax({
            url: url,
            beforeSend: function () {
    ...
    function lihat_mhs(kelas_id) {
        super_kelas_id = kelas_id;
        var url = "<?= site_url('dosen/dekan/update_penilaian/lihat_data_mahasiswa_uts') ?>/" + kelas_id;
        $.ajax({
            url: url,
            beforeSend: function () {
                $("#landing-nilai").html(loader);
    ...
    function pesan(kelas_id) {

        super_kelas_id = kelas_id;
        var url = "<?= site_url('dosen/penilaian/pesan_uts') ?>/" + kelas_id + "/prodi/dosen";
        console.log(url);
        $.ajax({
            url: url,
            success: function (res) {
                ...
                $("#pesan_" + kelas_id).addClass('btn-primary').removeClass('btn-danger').removeClass('badge-notif');
            }
        })
    }


    function validasi_uts() {
        var url = "<?= site_url('dosen/dekan/update_penilaian/validasi_uts') ?>/" + super_kelas_id;
        swal({
            title: '',
    ...
    function revisi() {
        var url = "<?= site_url('dosen/dekan/update_penilaian/revisi_uts') ?>/" + super_kelas_id;
        swal({
            title: '',
    ...
    function kelas() {
        var url = "<?= site_url('dosen/dekan/update_penilaian/kelas_uts') ?>";
        $.ajax({
            url: url,
            beforeSend: function () {
    ...
    function cari() {
        var url = "<?= site_url('dosen/dekan/validasinilai/cari_kelas') ?>";
    ...
    function show_history_comment(kelas_id) {
        var url = "<?= site_url('dosen/penilaian/show_history_comment_uts') ?>/" + kelas_id;
