                ...
                <form action="">
                    <select id="nilai-akademik" name="kode_nilai_akademik" class="form-control select2">
                        ...
                        <?php foreach ($tahun_akademik as $row) : ?>
                            <option <?= ($select == $row->kode_tahun_akademik ? "selected" : "") ?> value="<?= e($row->kode_tahun_akademik) ?>"><?= e($row->tahun_akademik) ?> - <?= e($row->semester == 0 ? "GENAP" : "GANJIL") ?></option>
                        <?php endforeach; ?>
                    </select>
                </form>
            </div>
...
<div class="modal fade bd-example-modal-lg" id="ModalNilaiValidasi" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h3 class="modal-title">Nilai Mahasiswa Tervalidasi Oleh Kaprodi</h3>
      </div>
      <div class="modal-body">
...
function show_nilai(param1,param2) {
    $.ajax({
        url: '<?= site_url('dosen/kaprodi/Validasinilai_kpat/revisi_nilai_validasi')?>',
        type: 'post',
        data: {
            "kelas": param1,
            "level": param2,
...
function show_nilai_validasi(param1,param2) {
    $.ajax({
        url: '<?= site_url('dosen/kaprodi/Validasinilai_kpat/revisi_nilai_divalidasi')?>',
        type: 'post',
        data: {
            "kelas": param1,
            "kode_ta" : $("#nilai-akademik").val(),
...
function show_mhs(param1,param2) {
    $.ajax({
        url: '<?= site_url('dosen/kaprodi/Validasinilai_kpat/revisi_nilai_validasi_mhs')?>',
        type: 'post',
        data: {
            "kelas": param1,
            "level": param2,
...
function selesai(param1,param2) {
    swal({
        title: '',
        html: "Menekan tombol <strong>Validasi Nilai</strong> berarti data nilai mahasiswa sudah selesai di periksa dan siap untuk di validasi oleh <strong>Dekan</strong>." +
        ...
        $.ajax({
            url: '<?= site_url('dosen/kaprodi/Validasinilai_kpat/revisi_nilai_selesai')?>',
            type: 'post',
            data: {
                "kelas": param1,
                "level": param2,
    ...
    swal({
        ...
        if ( pesan == '') {
            swal({
             ...
             $.ajax({
                url: '<?= site_url('dosen/kaprodi/Validasinilai_kpat/revisi_nilai_revisi')?>',
                type: 'post',
                data: {
                    "kelas": param1,
                    "level": param2,
...
function autoload2() {
    var url = "<?= site_url('dosen/kaprodi/validasinilai_kpat/choose_nilai_revisi') ?>";
    $.ajax({
        url: url,
        beforeSend: function () {
...
function refres() {
    const url = "<?= site_url('dosen/kaprodi/validasinilai_kpat/choose_nilai_revisi') ?>";
    $.ajax({
        url: url,
        data: {
