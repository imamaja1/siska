<nav>
  <div class="" id="nav-tab" role="tablist" style="text-decoration: none;" >
    <?php foreach ($kelas as $key => $value):?>
        <button class="nav-link btn btn-primary" id="nav-home_<?= e($value->level) ?>-tab" data-toggle="tab" data-target="#nav-home-<?= e($value->level) ?>" type="button" role="tab" aria-controls="nav-home" aria-selected="<?= ($key == 0) ? 'true':'false'; ?>">NIlai Ke - <?= e($value->level) ?> </button>
    <?php endforeach; ?>
  </div>
</nav>
<div class="tab-content" id="nav-tabContent">
    <?php foreach ($kelas as $no => $value):?>
        <div class="tab-pane fade" id="nav-home-<?= e($value->level) ?>" role="tabpanel" aria-labelledby="nav-home-tab">
