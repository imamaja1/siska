    ...
    <tbody>
        ...
        foreach ($data as $key => $row) :
            ...
            <tr>
                ...
                <td>
                    <?php
                        echo nilai_validasi($row->status_uts_dosen ? $row->status_uts_dosen : 0);
                    ?>
                </td>
                <td>
                    <?php 
                        echo nilai_validasi($row->status_uts_prodi ? $row->status_uts_prodi : 0);
                    ?>
                </td>
                <td>
                    <?php 
                        echo nilai_validasi($row->status_uts_dekan ? $row->status_uts_dekan : 0);
                    ?>
                </td>
                <td style="white-space: nowrap;width: 1px;">
                    <a class="btn btn-info btn-xs btn-flat" href="<?= site_url('dosen/update_nilai/nilai_mahasiswa_uts/' . $row->kelas_id) ?>"> <i class="fa fa-arrow-circle-right"></i>Update Nilai</a>
            </td>
            <td style="white-space: nowrap;width: 1px;">
