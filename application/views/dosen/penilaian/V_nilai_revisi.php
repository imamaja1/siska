<div class="row">
                ...
                <div class="box-header with-border">
                    <div style="size : 20px;"><b> KELAS - <?= $data_kelas->nama_kelas ?> </b>
                        ...
                        <div class="pull-right">
                            <a href="<?= site_url('dosen/penilaian/penilaian_revisi') ?>"
                                class="btn btn-success btn-xs flat"><i class="fa fa-arrow-circle-left"></i> Kembali</a>
                        </div>
                    </div>
                </div>
                <div>
                        ...
                        foreach ($semua_kelas as $key => $row):
                            ...
                            <tr>
                                ...
                                <td style="white-space: nowrap;width: 1px;">
                                    <button class="btn btn-success btn-xs btn-flat" data-toggle="modal"
                                        data-target="#ModalNilai"
                                        onclick="show_nilai(<?= $kelas_id ?>,<?= $row->level ?>)">Lihat Nilai</button>
                                    <?php if ($row->status_prodi == 'F'): ?>
                                        <button class="btn btn-danger btn-xs btn-flat"
                                            onclick="pembatalan(<?= $kelas_id ?>,<?= $row->level ?>)">Delete</button>
                                    <?php endif; ?>
                                    <?php
                                    if ($row->status_dekan == 'T') {
                                        ?>
                                        <a href="<?= site_url('dosen/penilaian/cetak_nilai_revisi_kelas/' . $row->id_kelas . '/' . $row->level . '/' . $ta) ?>"
                                            class="btn btn-warning btn-xs btn-flat"><i class="fa fa-print"></i> Cetak</a>
                                        <?php
                                    }
                                    ?>
                                </td>
    ...
    <?php if(count($semua_kelas) < 2 && false): ?>
    ...
    <div class="col-md-12">
        ...
        <?php endif; ?>
        <?php if ($this->session->flashdata('fail_download')): ?>
            <div class="alert alert-danger" role="alert">
                <?= $this->session->flashdata('fail_download'); ?>
            </div>
        <?php endif; ?>
        <div class="box box-info flat">
            <div class="box-header with-border">
                <h3 class="box-title"><b> KELAS - <?= $data_kelas->nama_kelas ?> </b>
              	...
              	<?php if(true): ?>
                <a href="<?= base_url('dosen/penilaian/excel/'.$kelas_id.'/'.$data_kelas->level) ?>" class="pull-right btn btn-sm btn-success">
                    Download Template
                </a>
                <a class="pull-right btn btn-sm btn-success" data-toggle="modal" data-target="#exampleModal" style="margin: 0 2px 0 2px ">
                    Import
                </a>
				<?php endif; ?>
                <br>
            ...
            <div class="box-body">
                  <?php if (!$persentasi_nilai): ?>
                  <div class="alert alert-danger" role="alert">
                      Jika Form Penginputan Data Tidak Tampil, Dimohon Untuk Mengisi <b>Presentasi Penilaian</b> Untuk Dapat Melakukan Penilaian Mahasiswa. 
                  </div>
                  <?php endif; ?>
                        ...
                        <tbody>
                            ...
                            foreach ($data as $key => $row):
                                ...
                                <tr>
                                    ...
                                    <td style="text-align:center"><p id="na<?= $key ?>"><?= trunc($row->na ? $row->na : $row->nilai_akhir,1) ?> </p></td>
                                    <td style="text-align:center"><p id="grade<?= $key ?>"><?= $row->grade ? $row->grade : '-' ?></p></td>
                                    <td style="text-align: center">
                                        <button class="btn <?= $row->ket ? "btn-success" : "btn-primary" ?> btn-xs btn-flat" id="ket<?= $key ?>" data-toggle="modal" data-target="#ModalKet" onclick="show_ket(<?= $row->kode_khs_detail ?>,<?= $kelas_id ?>,<?= $data_kelas->level ?>,<?= $key ?>)" <?= $data_kelas->status_dosen == 'T' ? 'disabled' : '' ?>></i><?= $row->ket ? "Ubah" : "Tambah" ?></button>
                                    </td>
...
<div class="modal fade bd-example-modal-lg" id="ModalNilai" tabindex="-1" role="dialog"
...
<div class="modal fade " id="ModalKet" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"
    aria-hidden="true">
      ...
      <form action="<?= base_url('dosen/penilaian/import_excel/'.$kelas_id.'/'.$data_kelas->level) ?>" method="post" enctype="multipart/form-data">
        <div class="modal-body">
            <div class="mb-3">
                ...
                <input class="form-control" type="file" name="file_excel" id="formFile" accept=".xls,.xlsx" required>
            </div>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
...
<script>
    function nilai(id, kelas, key, level) {
        $.ajax({
            url: '<?= site_url('dosen/penilaian/nilai_revisi') ?>',
            type: 'post',
            data: {
                "kelas": kelas,
                "level": level,
            ...
            success: function (data) {
                if (jQuery.parseJSON(data).status) {
                    ...
                    $("#na" + key).text(jQuery.parseJSON(data).data.na);
                } else {
                    swal({
                        title: '',
    ...
    function selesai(param1, param2, param3) {
        ...
        swal({
            ...
            $.ajax({
                url: '<?= site_url('dosen/penilaian/revisi_dosen_selesai') ?>',
                type: 'post',
                data: data,
                success: function (data) {
                    location.reload();
    ...
    function new_penilaian(param1, param2) {
        swal({
            ...
            $.ajax({
                url: '<?= site_url('dosen/penilaian/revisi_new_penilaian') ?>',
                type: 'post',
                data: {
                    "kelas": param1,
                    "level": param2,
    ...
    function pembatalan(param1, param2) {
        swal({
            ...
            $.ajax({
                url: '<?= site_url('dosen/penilaian/revisi_pebatalan_penilaian') ?>',
                type: 'post',
                data: {
                    "kelas": param1,
                    "level": param2,
    ...
    function show_nilai(param1, param2) {
        $.ajax({
            url: '<?= site_url('dosen/penilaian/revisi_nilai_mahasiswa') ?>',
            type: 'post',
            data: {
                "kelas": param1,
                "level": param2,
    ...
    function show_ket(param1, param2, param3, param4) {
        ...
        this.key_ket = param4;
        $.ajax({
            url: '<?= site_url('dosen/penilaian/revisi_ket') ?>',
            type: 'post',
            data: {
                "id": param1,
                "kelas": param2,
    ...
    function simpan_ket() {
        $.ajax({
            url: '<?= site_url('dosen/penilaian/revisi_ket_val') ?>',
            type: 'post',
            data: {
                "id": id,
                "kelas": kelas,
