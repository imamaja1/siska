    ...
    <div class="col-md-12">
        <div class="box box-info flat">
            <div class="box-header with-border">
                ...
                <small class="pull-right text-muted">Perbandingan Nilai Penilaian vs KHS | Pengisian ke-<?= e($data_kelas->level) ?></small>
            </div>
            <div class="box-body">
                            ...
                            <?php $no = 1; foreach ($data as $row): ?>
                                <tr>
                                    ...
                                    <td style="text-align:center">
                                        <b><?= e(trunc($row->na ? $row->na : $row->nilai_akhir, 1)) ?> (<?= e($row->grade ?? '-') ?>)</b>
                                    </td>
                                    <td style="text-align:center">
                                        <b><?= e($row->khs_na) ?> (<?= e($row->khs_grade ?? '-') ?>)</b>
                                    </td>
                                    <td style="text-align:center">
                                        <?php $na_pen = trunc(($row->na ? $row->na : $row->nilai_akhir), 1); ?>
                                        <?php $sync = ($na_pen == $row->khs_na); ?>
                                        <span class="badge <?= $sync ? 'bg-green' : 'bg-red' ?>"><?= $sync ? 'Sinkron' : 'Belum Sinkron' ?></span>
                                        <?php if ($row->block_id): ?><br><small class="text-muted">Block</small><?php endif; ?>
                                        <?php if ($row->mbkm_id): ?><br><small class="text-muted">MBKM</small><?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
